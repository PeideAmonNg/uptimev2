...
function getUserStatusById(userid, limit = 50) {
  return User.findOne({id: userid}, {username: 1, name: 1}).then(user => {
    ...
    return Promise.all([
      user.name,
      UserStatus.find({username: user.username}, {username: 1, createdAt: 1, status: 1}).sort({createdAt: -1}).limit(limit).lean()
    ]);
  });
}
...
module.exports = async (req, res) => {
  ...
  let userStatuses = [];
  if(req.query.id) {
    [name, userStatuses] = await getUserStatusById(req.query.id);    
  }
  console.log(userStatuses.length);
  res.status(200).send({name, userStatuses});
};